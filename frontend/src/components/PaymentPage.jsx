...
export default function PaymentPage({ amount, name, email, phone, purpose, memberId, eventRegistrationId, membershipType, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    axios.get(`${API}/public/payment-settings`).then(r => {
      ...
      else if (r.data.upi_ids?.length) setTab('upi');
  ...
  const handleRazorpay = async () => {
    ...
    try {
      ...
      if (!razorpay_order_id) { setError('Razorpay not configured. Please use UPI or Bank Transfer.'); setRzpLoading(false); return; }

      const loaded = await loadRazorpay();
      if (!loaded) { setError('Failed to load Razorpay. Try UPI or Bank Transfer.'); setRzpLoading(false); return; }
  ...
  if (paySettings?.upi_ids?.length) tabs.push({ key: 'upi', label: 'UPI', icon: QrCode });
  ...
  if (tabs.length === 0) return (
    <div style={{ textAlign: 'center', padding: '40px' }}>
      ...
      <p style={{ color: '#6b7280', fontSize: '14px' }}>No payment methods configured. Please contact IDSEA for payment instructions.</p>
  ...
  return (
    <div data-testid="payment-page">
      ...
      <div style={{ textAlign: 'center', padding: '20px 0 24px', borderBottom: '1px solid #e5e7eb', marginBottom: '24px' }}>
        ...
        <div style={{ fontSize: '32px', fontWeight: 800, color: '#0c3c60', fontFamily: 'Poppins' }} data-testid="payment-amount">₹{amount?.toLocaleString('en-IN')}</div>
      ...
      {tab === 'razorpay' && (
        <div style={{ textAlign: 'center' }}>
          <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '20px' }}>Pay securely via Credit/Debit Card, Net Banking, or UPI through Razorpay</p>
          <button onClick={handleRazorpay} disabled={rzpLoading} className="btn-primary" data-testid="pay-razorpay-btn"
            ...
            {rzpLoading ? 'Processing...' : `Pay ₹${amount?.toLocaleString('en-IN')}`}
          </button>
